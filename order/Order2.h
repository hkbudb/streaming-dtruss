...
//only maintain the ordered_layers, do not maintain ordered_list

#ifndef ORDER_ORDER2_H
#define ORDER_ORDER2_H
#include "common.h"
#include "queue"
...
public:
    Order2(vector<pair<uint32_t, uint32_t> > & vEdges, uint32_t uiMaxN, uint32_t uiMaxM,
          uint32_t uiKc, uint32_t uiKf);
    Order2(const Order2&) = delete;
    Order2& operator=(const Order2&) = delete;
