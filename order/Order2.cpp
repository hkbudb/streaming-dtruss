...
/**
    find neighbor
**/
void Order2::findNeib(vector<AdjEntry> &vAdj1, vector<AdjEntry> &vAdj2, vector<pair<uint32_t, uint32_t> > & vTrdE)
{
    size_t p1 = 0, p2 = 0;
 ...
 **/
void Order2::removeItem(vector<uint32_t> &v, uint32_t item) {
...
**/
void Order2::compressUpdatedLayers() {
...
**/
void Order2::insertNewLayer(uint32_t eid) {
...
**/
void Order2::findLayerToInsert(vector<uint32_t> &support_cnt_c, vector<uint32_t> &support_cnt_f, uint32_t eid) {
    uint32_t layer_id = 0;
...
**/
void Order2::insertNeighborsToQueue(vector<bool> &be_in_pq, PQ &m_PQ, vector<vector<pair<uint32_t, uint32_t>>> &neighbors_of_different_layer,
...
//    }
    ...
    cout << "start to insert " << lower_bound << " " << upper_bound << " " << neighbors_of_different_layer.size() << endl;
   // lower_bound = 0;
...
**/
uint32_t Order2::h_index(vector<::uint32_t> &neigh_cycle_support) {
...
/**
    init
**/
Order2::Order2(vector<pair<uint32_t, uint32_t> > & vEdges, uint32_t uiMaxN, uint32_t uiMaxM, uint32_t uiKc, uint32_t uiKf)
{
    m_ui_n = uiMaxN;
    m_ui_m = uiMaxM;
    m_ui_kc = uiKc;
...
**/
bool Order2::findEid(uint32_t x, uint32_t y, uint32_t *piEid)
{
...
/**
    rename pid
**/
uint32_t Order2::renamePid(uint32_t x)
{
    uint32_t u = 0;
    if (m_v_bPid[x])
    {
...
/**
    ...
    return eid
**/
bool Order2::addEInfo(uint32_t x, uint32_t y, uint32_t *puiEid)
{
    /* rename */
...
/**
    add one edge info
**/
uint32_t Order2::insertDT(vector<uint32_t> &vInsE)
{
    ...
    for (uint32_t eid : vInsE)
    {
        ...
        while (!m_PQ.empty()){
            cout << "insert test 4 " << m_PQ.top().eid << endl;
            uint32_t cur_eid =  m_PQ.top().eid;
            m_PQ.pop();
            be_in_pq[cur_eid] = true;
            cout << "insert test 4.1 " << cur_eid << " " <<  m_vv_ordered_list[cur_eid].layer_number << " " << m_mu <<  endl;
...
//            else{
    ...
    cout << "insert test 18" << endl;
    return vResE.size();
}
/**
    add edges
**/
void Order2::add(vector<pair<uint32_t, uint32_t> > & vEdges)
{
...
/**
    save community
**/
void Order2::save(vector<pair<uint32_t, uint32_t> > & vEdges)
{
...
**/
pair<double,double> Order2::getDtQuality(vector<pair<uint32_t, uint32_t>> & vEdges)
{
...
**/
uint32_t Order2::rmDT(vector<uint32_t> &vRmE)
{
    //order based method
...
//                cout << "test 7.1" << endl;
//                ASSERT_MSG(original_layer == m_mu, "layer info wrong " << original_layer << " " << m_mu);
//            }
...
/**
    remove edges
**/
void Order2::remove(vector<pair<uint32_t, uint32_t> > & vEdges)
{
    uint32_t eid = 0;
...
**/
bool Order2::verify(uint32_t peel_layer_number, uint32_t eid,  uint32_t &layer_number_test){
    uint32_t x = 0, y = 0;
...
**/
void Order2::peeling(vector<uint32_t> &vEdges, vector<uint32_t> &vFixE, vector<uint32_t> &vResE)
{
    const auto beg = std::chrono::steady_clock::now();
...
**/
void Order2::findNeibAndLayer(vector<Order2::AdjEntry> &vAdj1, vector<Order2::AdjEntry> &vAdj2, vector<pair<uint32_t, uint32_t>> &vTrdE,
...
**/
//bool Order2::orderPrecede(uint32_t eid_x, uint32_t eid_y) {
