...
/**
    find edges in [ts, te]
**/
void Stream::getEdges(uint32_t uiTS, uint32_t uiTE, vector<ST_EDGE_ENTRY> &vDesEdges)
{
    vector<ST_EDGE_ENTRY>::iterator itLowB;
    vector<ST_EDGE_ENTRY>::iterator itUpB;

    ASSERT(uiTS <= uiTE);
    itLowB = upper_bound(m_vE.begin(), m_vE.end(), (ST_EDGE_ENTRY){uiTS, 0, 0}, cmp);
    itUpB = upper_bound(m_vE.begin(), m_vE.end(), (ST_EDGE_ENTRY){uiTE + 1, 0, 0}, cmp);

    while (itLowB != itUpB)
    {
        vDesEdges.push_back((ST_EDGE_ENTRY){itLowB->t, itLowB->x, itLowB->y});
        ++itLowB;
    }
}
/**
