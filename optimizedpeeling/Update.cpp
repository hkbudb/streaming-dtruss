...
/**
    add one edge info
**/
uint32_t Update::insert_predict_DT(vector<uint32_t> &vInsE, int &iStart)
{
    list<uint32_t> lsQ;
    ...
    {   
        ...
        //if(uiCSup >= m_ui_kc && uiFSup >= m_ui_kf)
        //{
            lsQ.push_back(eid);
        //}
    }
    ...
    for(uint32_t uiCurEid = 0; uiCurEid < m_ ; uiCurEid++)
    {
        if(m_v_EInfo[uiCurEid].bUsed)
        {
            ...
            //ASSERT_MSG(index < 0 || index > m_v_EInfo[uiCurEid].lifeTime.size(), "index out of range "<< index << " " << m_v_EInfo[uiCurEid].t << " " << iStart <<  " " << m_v_EInfo[uiCurEid].lifeTime.size() << " " << COMMON_RUP((m_v_EInfo[uiCurEid].t - iStart),m_ui_step));
...
/**
    add one edge info
**/
uint32_t Update::insert_all_DT(vector<uint32_t> &vInsE, int &iStart) {
    list<uint32_t> lsQ;
    vector<pair<uint32_t, uint32_t> > vCE;
    vector<pair<uint32_t, uint32_t> > vFE;

    //update the life-time support information for all edges
    for (uint32_t eid : vInsE)
    {
        uint32_t x = m_vp_edges[eid].first;
        uint32_t y = m_vp_edges[eid].second;
    ...
    //conduct BFS to collect all edges reachable from the inserted edges with enough support
    while (!lsQ.empty())
    {
        ...
        int index = m_v_EInfo[uiCurEid].lifeTime.size() - COMMON_RUP((m_v_EInfo[uiCurEid].t - iStart), m_ui_step);
        uint32_t kc_of_cur_vertex = m_v_EInfo[uiCurEid].lifeTime[index].first;
        uint32_t kf_of_cur_vertex =  m_v_EInfo[uiCurEid].lifeTime[index].second;
        if(kc_of_cur_vertex < m_ui_kc || kf_of_cur_vertex < m_ui_kf) {
            vbInPool[uiCurEid] = true;
            continue;
        }
        ...
        uint32_t uiCSup = vCE.size();
        //uint32_t uiCSup = kc_of_cur_vertex;
        ...
        uint32_t uiFSup = vFE.size();
        //uint32_t uiFSup = kf_of_cur_vertex;
...
//        }
//    }
//    vTargetE = pruned_targe_edges_support_vec;
...
void Update::add(vector<ST_EDGE_ENTRY> & vEdges, int &iStart)
{
    ...
    m_uiDTCnt += /*insert_predict_DT*/insert_all_DT(vInsE, /*realStartTime*/iStart );
